...
public abstract class SilkAdapter<ItemType extends SilkComparable> extends BaseAdapter implements ScrollStatePersister {
    ...
    private ItemType assignId(ItemType item) {
        if (getCount() == 0) item.setSilkId(1);
        else item.setSilkId(getItem(getCount() - 1).getSilkId() + 1);
        return item;
    }
    ...
    public void add(int index, ItemType toAdd) {
        isChanged = true;
        this.mItems.add(index, assignId(toAdd));
    ...
    public void add(ItemType toAdd, boolean updateList) {
        isChanged = true;
        this.mItems.add(assignId(toAdd));
    ...
    /**
     * Updates a single item in the adapter using equalTo() from SilkComparable. Once the filter finds the item, the loop is broken
     ...
     *
     * @param comparator An optional comparator to sort the list with before setting it to the adapter.
     */
    public final void set(ItemType[] toSet, Comparator<ItemType> comparator) {
        if (toSet == null) clear();
        else set(new ArrayList<ItemType>(Arrays.asList(toSet)), comparator);
    }
     ...
     * @param comparator An optional comparator to sort the list with before setting it to the adapter.
     */
    public final void set(List<ItemType> toSet, Comparator<ItemType> comparator) {
        isChanged = true;
        this.mItems.clear();
        if(comparator != null)
            Collections.sort(toSet, comparator);
        if (toSet != null) {
    ...
    /**
     * Removes a single item in the adapter using equalTo() from SilkComparable. Once the filter finds the item, the loop is broken
    ...
    /**
     * Removes an array of items from the adapter, uses equalTo() from SilkComparable to find the items.
     */
    public final void remove(ItemType[] toRemove) {
